...
def query(query,
    ...
    json = request.get_json(url, headers={'User-Agent': useragent})
...
@hook.hook('command', ['duckduckgo', 'ddg'])
async def ddg(bot, msg):
    inp = msg.message.replace('my ip', '').replace('ip address', '').replace('addr', '')
    if inp == 'ip':
        create_task(bot.send_privmsg([msg.target], '0.0.0.0'))
    create_task(bot.send_privmsg([msg.target], get_zci(inp)))
