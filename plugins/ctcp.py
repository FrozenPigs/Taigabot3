...
async def finger(bot, message):
    ...
    if message.command == '\x01FINGER\x01':
        asyncio.create_task(
            bot.send_notice([message.user.nickname], "\x01FINGER pls don't finger me\x01"))
...
async def version(bot, message):
    ...
    if message.command == '\x01VERSION\x01':
        try:
            asyncio.create_task(
                bot.send_notice([message.user.nickname], f'\x01VERSION TaigaBotNet Version 3.8\x01'))
...
async def userinfo(bot, message):
    ...
    if message.command == '\x01USERINFO\x01':
        asyncio.create_task(bot.send_notice([message.user.nickname], '\x01USERINFO immabot\x01'))
...
async def clientinfo(bot, message):
    ...
    if message.command == '\x01CLIENTINFO\x01':
        ...
        asyncio.create_task(
            bot.send_notice([message.user.nickname], f'\x01CLIENTINFO {supported}\x01'))
...
async def ctcptime(bot, message):
    ...
    if message.command == '\x01TIME\x01':
        ...
        asyncio.create_task(bot.send_notice([message.user.nickname], f'\x01TIME {curtime}\x01'))
