...
options.add_option(
    '--logfile', '-o', dest="log_file", default="results.txt", help="Optional logfile destination")
options.add_option(
    '--resume',
    dest="resume",
    action="store_true",
...
options.add_option(
    ...
    dest="timeout",
    default=5,
    help="How long to wait for remote host to respond before timing out")
options.add_option(
    '--json', dest="json_file", default=None, help="Save data as json into this file")
options.add_option(
    '--only-vulnerable',
...
options.add_option(
    ...
    action="store_true",
    default=False,
    help="Print verbose information to screen")
options.add_option(
    '--max',
    dest="max",
    default=None,
    help="Exit program after scanning X hosts. Useful with --only-unscanned")
...
@hook.hook('command', ['heartbleed'])
async def heartbleed(bot, msg):
    "bash <id> -- Gets a random quote from Bash.org, or returns a specific id."
    global host_status
    host_status = {}

    scan_host(msg.message)

    create_task(bot.send_privmsg([msg.target], print_summary()))
