...
def _pull_track_plays(api_key, track_title, track_artist, lastfm_nick):
    ...
    url += f'&track={track_title}&artist={track_artist}&api_key={api_key}'
    response = requests.get(url)
    if response.status_code == 200:
        try:
...
def _output_nowplaying(client, data, lastfm_nick, title, artist, album, plays, tags):
...
async def nowplaying(client, data):
    ...
    try:
        ...
        asyncio.create_task(
            client.message(
                data.target,
                'No last.fm API key found. Please create an API account, add it to \'api_keys\' in config.json with the tag \'last_fm\''))
        return
    
    # dealing with getting the last.fm nickname
    ...
    #final output
    _output_nowplaying(client, data, lastfm_nick, title, artist, album, plays, tags)
