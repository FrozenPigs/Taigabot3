...
japanese_characters = ''.join([str(aLetter) for aLetter in all_letters])
...
@hook.hook('command', ['trans', 'translate'])
async def translate(bot, msg):
    ...
    create_task(bot.send_privmsg([msg.target], f'[{label}] {result}'))
