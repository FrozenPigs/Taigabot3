...
hashtag_db_ready = False


def init_hashtag_db(bot):
    global hashtag_db_ready
    db.init_table(bot.db, 'hashtags', ['word', 'data', 'nick'], ['word'])
    hashtag_db_ready = True
    print('Hashtag Database Ready')
...
# @hook.regex(r'(.*) is (.*)')
# @hook.regex(r'(.*) are (.*)')
@hook.hook('command', ['r', 'lean', 'remember'])
async def remember(bot, msg):
